...
- `sameSite`: a boolean or string indicating whether the cookie is a "same site" cookie (`false` by default). This can be set to `'strict'`, `'lax'`, `'none'`, or `true` (which maps to `'strict'`). If set to `'none'`, also set `'secure'` to `true`, otherwise the sameSite option will not be respected. `'none'` has some browser incompatibilities which must be taken into account.
